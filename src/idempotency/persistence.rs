...
// Tell `sqlx` that the name of `header_pair[]` is `_header_pair`, as that is what
// Postgres generates for array types in `CREATE TABLE` statements.
...
pub async fn save_response(
    ...
    // Take apart the original HTTP response, so that we get ownership of the ` body`
    let (response_head, body) = http_response.into_parts();
    // `MessageBody::Error` is not `Send` + `Sync`, so it doesn't play nicely with `anyhow`
