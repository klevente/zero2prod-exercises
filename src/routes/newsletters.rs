...
pub async fn publish_newsletter(
    ...
    // attach the received username to the current `Span`
    tracing::Span::current().record("username", &tracing::field::display(&credentials.username));

    let user_id = validate_credentials(credentials, &pool).await?;
    // attach the found user id to the current `Span`
        ...
        match subscriber {
            Ok(subscriber) => {
                email_client
                    ...
                    .await
                    // lazy version of context, which only calls the closure when an error has occurred
                    // this is good because `format!` allocates on the heap
                    .with_context(|| {
            ...
            Err(error) => {
                ...
                "Skipping a confirmed subscriber as their stored contact details are invalid.");
            }
        }
    }
...
async fn validate_credentials(
    ...
    // create a dummy user and password to verify in order to mitigate a timing attack
